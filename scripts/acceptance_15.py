...
def build_checks() -> list[CheckCommand]:
    ...
    return [
        ...
        CheckCommand("local-client", "Remote terminal static contract", (PYTHON_BIN, "-m", "pytest", "tests/unit/test_remote_terminal_static_contract.py", "-q"), env=python_env),
        ...
        CheckCommand("runtime", "Trace store unit tests", (PYTHON_BIN, "-m", "pytest", "tests/unit/test_trace_store.py", "-q"), env=python_env),
        CheckCommand("runtime", "Feedback loop unit tests", (PYTHON_BIN, "-m", "pytest", "tests/unit/test_feedback_loop.py", "-q"), env=python_env),
        ...
        CheckCommand("self-hosted", "24h soak runner contract", (PYTHON_BIN, "-m", "pytest", "tests/unit/test_self_hosted_24h_soak.py", "-q"), env=python_env),
        CheckCommand(
            "self-hosted",
            "Release acceptance report parity gate",
            (
                PYTHON_BIN,
                "-m",
                "pytest",
                ...
                "-q",
                "-k",
                "six_layer or replay_layer or control_proposal or observer_parity",
            ),
            env=python_env,
        ),
