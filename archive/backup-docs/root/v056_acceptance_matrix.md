...
| memory / habit / relation / 记忆-习惯-关系 | partial | `§9-§10 Habit/Hippocampus; relation touchpoints in §3/§5 -> src/nalr/memory/store.py -> hot/warm/archive tiers, v0.56 decay/interference gates, cue-quality recovery, suppressed_recoverable habit state, relation trace/closeness persist, and 2026-04-04 cue normalization / identity family clustering / schema migration are live; tests/simulation/test_memory_and_habit.py tests/simulation/test_memory_tiers.py tests/unit/test_memory_store.py` pass, but evidence-pointer archive compaction is still missing. |
...
| observer / analytics / 观测与分析 | partial | `§15-§16 proposal_trace/调试分析 -> services/observer/api/app.py; services/observer/dashboard/index.html -> read-only API, skill/conflict/mode/ablation views exist, and 2026-04-04 diagnostics (`/diagnostics/state-delta`, `/diagnostics/identity-blockers`, `/diagnostics/cue-fragmentation`, `/diagnostics/run-contamination`, `/diagnostics/why-no-change/{round_ref}`, `/diagnostics/migration`) are exposed; tests/integration/test_observer_api.py and tests/integration/test_observer_diagnostics.py` pass, but dashboard depth and analytics storage are incomplete. |
...
| Spec Item / 规格项 | Status / 实现状态 | Test / Trace Evidence / 测试或追踪证据 |
|---|---|---|
| `xi_emo`、`xi_mood`、`lambda_noise`、`r_intensity` 落地 / Core stochastic variables | implemented | `§5.4-§5.10 情境随机层 -> src/nalr/runtime/controller.py::_apply_stochastic_layer() -> StochasticState stores emo_channel, xi_emo, xi_mood, lambda_noise, r_intensity, round_seed; entropy sampling is now backed by src/nalr/runtime/entropy.py::MacOSSystemEntropyProvider using os.urandom(); tests/unit/test_formula_runtime_alignment.py` confirms stochastic_state is persisted. |
| KL / noise guard 与 trace 字段 / KL-noise guard and trace coverage | implemented | `§5.12 调试与安全边界 -> src/nalr/runtime/controller.py::_apply_stochastic_layer() -> KL divergence is computed and noise_guard_triggered is set when over threshold; stochastic trace fields now carry local-system entropy refs with source=macos_os_urandom and endpoint=file:///dev/urandom; tests/unit/test_runtime_controller.py` covers trace consistency and local entropy refs. |
...
| cue 清洗、identity 聚类与迁移摘要 / Cue normalization, identity clustering, and migration summary | implemented | `§9-§10 Habit/Hippocampus runtime hygiene -> src/nalr/memory/store.py::_derive_cue(); _normalize_existing_cue(); identity_evidence(); migration_report() -> invalid freeform cue filtering, identity:self_probe/name_probe/name_origin clustering, legacy cue merge, and migration status reporting are live; tests/unit/test_memory_store.py` covers the new normalization and migration behavior. |
...
| state delta / identity / run contamination 诊断链 / State-delta, identity, and run-contamination diagnostics | implemented | `§15 proposal_trace drill-down -> services/observer/api/app.py; src/nalr/runtime/controller.py::state_delta_timeline(); identity_blockers(); cue_fragmentation_report(); run_contamination_report(); why_no_change(); migration_report() -> observer can now explain no-change cases and identity blockers from canonical trace fields; tests/integration/test_observer_diagnostics.py` covers these endpoints. |
