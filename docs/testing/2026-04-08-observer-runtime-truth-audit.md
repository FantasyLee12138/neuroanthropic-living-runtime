...
- `Think_Like_Human(TLH)_v1.0.md` 当前已经退化为兼容入口，正文明确把有效 TLH 内容指向 `v1.5`。
...
| 1.5 要有长期在线证据，不能只做短时真相对齐 | `partial` | 当前只有 heartbeat / stalled contract 和 launcher 收敛，没有新增 24h soak 产物 | 本轮未执行 24h soak | 只做了短时隔离实例 live-ish 验收 | 与 [`PLANS.md`](/Users/fantasylee/类脑架构/PLANS.md) 一致，`24h 运行证据` 仍然是 in-flight 项 |
...
- `PLANS.md` 当前没有旧版 `Doing` 清单，而是 1.5 ledger 结构；本审计按现行 ledger 解释“已交付 / in-flight”，没有再回写旧格式状态。
